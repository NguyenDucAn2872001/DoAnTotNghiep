...
<script setup>
  ...
  const save= function (format) {
    console.log("4");
    ref.doceditcontainer.ej2Instances.documentEditor.save( ref.doceditcontainer.ej2Instances.documentEditor.documentName == '' ? 'sample' :  this.$refs.doceditcontainer.ej2Instances.documentEditor.documentName, format);
  }
  ...
  const printBtnClick= function () {
    console.log("6");
    ref.doceditcontainer.ej2Instances.documentEditor.print();
  }
  ...
  const titleBarBlurEvent = function () {
    ...
    document.getElementById("documenteditor_title_contentEditor").contentEditable = 'false';
    ref.doceditcontainer.ej2Instances.documentEditor.documentName = document.getElementById("documenteditor_title_name").textContent;
  }
  ...
  const documentChangedEvent= function () {
    console.log("11");
    var obj = this.$refs.doceditcontainer.ej2Instances.documentEditor;
    documentTitle = obj.documentName === '' ? 'Untitled Document' : obj.documentName;
    document.getElementById("documenteditor_title_name").textContent = obj.documentName ;
    setTimeout(() => { obj.scrollToPage(1); }, 10);
  }
  const updateDocumentEditorSize= function () {
    ...
    var windowWidth = window.innerWidth -18;
    var windowHeight = window.innerHeight - (document.getElementById("documenteditor_titlebar").offsetHeight +26);
    ref.doceditcontainer.ej2Instances.resize(windowWidth, windowHeight);
  ...
  const openTemplate=function () {
    ...
    uploadDocument.append('DocumentName', 'Getting Started.docx');
    var loadDocumentUrl =
      ref.doceditcontainer.ej2Instances.serviceUrl + 'LoadDocument';
    var httpRequest = new XMLHttpRequest();
    httpRequest.open('POST', loadDocumentUrl, true);
    httpRequest.onreadystatechange = function () {
      if (httpRequest.readyState === 4) {
        if (httpRequest.status === 200 || httpRequest.status === 304) {
          ref.doceditcontainer.ej2Instances.documentEditor.open(
            httpRequest.responseText
          );
        }
      }
  ...
  nextTick( ()=> {
    ref.doceditcontainer.ej2Instances.locale = 'en-US';
    ref.doceditcontainer.ej2Instances.serviceUrl = serviceUrl;
    ref.doceditcontainer.ej2Instances.documentChange = () => {
