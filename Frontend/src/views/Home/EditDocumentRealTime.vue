...
const hasFetchedContent=ref(false)
const contentFinal=ref("")

const ListUser=ref([])

onMounted(async()=>{
    console.log(getIdDocument , typeof(getIdDocument));
  setTimeout(function(){   
      Loading.value=false
  }, 1500);
  await getInfoUser()
    ...
    await axios.get(import.meta.env.VITE_GETAPI_USER).then(response =>
    {
      for (let i = 0; i < response.data.length; i++) {
        if(response.data[i].id==getid){
            ...
            IdInput.value=response.data[i].id
        }
      }
      for (let i = 0; i < response.data.length; i++) {
            if (response.data[i].id!=getid) {
...
const editRealtime = async() => {
  ...
  editor.value = new Editor({
      extensions: [
      Document,Paragraph,Text,Bold,BulletList,ListItem,Color,TextStyle,Image,Dropcursor,Gapcursor,TableRow,TableHeader,TableCell,
      Table.configure({
        resizable: true,
      }),
      Heading.configure({
      ...
      content:hasFetchedContent.value ? "" : await getContentDocumentRealTime()
  });
  editor.value.on('update', () => {
      contentFinal.value = editor.value.getHTML()
      updateDocument(editor.value.getHTML())
  })


}

const getContentDocumentRealTime = async () => {

    try {
    const response = await axios.get(import.meta.env.VITE_GET_CONTENT_DOCUMENT_REAL_TIME, { params: { id: getIdDocument } });
    console.log(response.data ,"test get content");
    const contentDocument = response.data[0].content;
    return contentDocument;
    } catch (error) {
    console.log(error);
    return ""; // Trả về giá trị mặc định nếu xảy ra lỗi
    }
};

const updateDocument=(content)=>{
    try {
        axios.put(import.meta.env.VITE_UPDATE_CONTENT_DOCUMENT_REAL_TIME,{
            id:getIdDocument,
            content:content
        }).then(response =>
        {
